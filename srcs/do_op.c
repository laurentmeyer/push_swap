...
int		op_push(t_data *data, char stack)
{
	t_stack	*src;
	t_stack	*dst;

	dst = stack == 'a' ? data->a : data->b;
	src = stack == 'a' ? data->b : data->a;
	if (src->count > 0)
		push(dst, pop(src));
	return (1);
}
...
int		op_rotate(t_stack *stack)
{
	...
	if (stack->count > 0)
	{
		i = (stack->data)[stack->count - 1];
		ft_memmove(stack->data + 1, stack->data, (stack->count - 1) * sizeof(int));
		(stack->data)[0] = i;
	}
	return (1);
}
...
int		op_reverse_rotate(t_stack *stack)
{
	...
	{
		i = (stack->data)[0];
		ft_memmove(stack->data, stack->data + 1, (stack->count - 1) * sizeof(int));
		(stack->data)[stack->count - 1] = i;
	}
	return (1);
}
...
int		do_op(t_data *data, char *s)
{
	if (ft_strequ(s, "sa"))
		...
		return (op_push(data, 'a'));
	else if (ft_strequ(s, "pb"))
		return (op_push(data, 'b'));
